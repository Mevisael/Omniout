 ...
 * Build: v234.0.0
 */
import { handleDataCards, doGenericInvokeWithCont } from "c/salesforceUtils";
...
function getCardsDataHandler(requestData) {
  ...
  if (requestData) {
    ...
    const useContinuation =
      requestObj.value.optionsMap &&
      typeof requestObj.value.optionsMap === "string"
        ? JSON.parse(requestObj.value.optionsMap).useContinuation
        : false;
    if (
      useContinuation &&
      ...
      return doGenericInvokeWithCont({ dataSourceMap: requestData })
        .then(res => {
      ...
      return promise;
    } else if (requestObj.type.toLowerCase() === "integrationprocedures") {
