...
import assistiveText from "@salesforce/label/c.accordionAssistiveText";
...
export default class AccordionSection extends LightningElement {
  ...
  @api title = "";
  /**
   * @type {Object} - It contains index of the section and total count of all sections.
   * @scope api (public)
   */
  @api sectionData;
  /**
  ...
  get assistiveText() {
    if (this.helpText) {
      let text = this.helpText.replace(/\{0\}/gi, this.sectionData.count);
      return text.replace("{1}", this.sectionData.index);
    }
    return "";
  }
