...
import {
  ...
  checkPermission,
  ErrorToastTitle,
  AppendRecordNoRecordsError,
  AppendRecordTypeError,
  getflexCardMetadata
...
export const FlexCardMixin = Base =>
  class extends Base {
    ...
    @api set cardNode(val) {
      if (val) {
        this._dataNode = val;
        //Making sure attributes order should not matter and setRecords gets called again for cardNode
        if (this.setRecordsTempValue) {
          this.setRecords(this.setRecordsTempValue);
        }
      }
    }
    get cardNode() {
    ...
    @api set parentRecords(val) {
      if (val) {
        this._parentRecords = Array.isArray(val) ? [...val] : { ...val };
        this.processParentRecord();
      }
    }
    get parentRecords() {
      return this._parentRecords;
    }

    @api set parentAttribute(val) {
      if (val) {
        this._originalOmniAttributes["parent-attribute"] = { ...val };
    ...
    setRecords(value) {
      ...
      if (value && (this._dataNode || this._parentData)) {
        ...
        // set this temp value if dataNode and parentData is undefined
        this.setRecordsTempValue = value;
      }
    }
    ...
    getPageReferenceParameters(currentPageReference) {
      if (!isEqual(this._currentpageRef, currentPageReference)) {
        this._currentpageRef = currentPageReference;
        if (
          this._unInterpolatedCardData &&
          this._currentpageRef &&
          this._currentpageRef.state &&
          this.isCardUpdateNeeded()
        ) {
          this.setDefinition(cloneDeep(this._unInterpolatedCardData));
        }
      }
    }

    isCardUpdateNeeded() {
      for (const param in this._currentpageRef.state) {
        if (this.dataStr && this.dataStr.indexOf("Params." + param) > -1) {
          return true;
        }
      }
      return false;
    ...
    //Omniscript dynamic update support
    _omniJsonData;
    _setupOmniListener = false;
    _listenOmniAttributes = [];
    _originalOmniAttributes = {};
    _preventSaveState = false;
    @api listenOsDataChange = false;

    @api get omniJsonData() {
      return this._omniJsonData;
    }

    set omniJsonData(val) {
      if (val && this.listenOsDataChange) {
        this._omniJsonData = val;
        if (!this._setupOmniListener) {
          this.setupOmniListener();
        }
        if (this._setupOmniListener) {
          this.updateOmniAttributes();
        }
      }
    }

    //dynamic update of os attributes passed on to cards [records,parentAttribute]
    updateOmniAttributes() {
      if (this._listenOmniAttributes?.length > 0) {
        this._listenOmniAttributes.forEach(attr => {
          let value = this.omniGetMergeField(attr.source);
          if (
            attr.name === "records" &&
            JSON.stringify(this._originalOmniAttributes["records"]) !== value
          ) {
            this.setRecords(JSON.parse(value));
          } else if (
            attr.name === "parent-attribute" &&
            JSON.stringify(this._originalOmniAttributes["parent-attribute"]) !==
              value
          ) {
            this._parentAttrbute = JSON.parse(value);
            this._originalOmniAttributes["parent-attribute"] = {
              ...this._parentAttrbute
            };
            this._preventSaveState = true;
            if (
              typeof this._originalOmniAttributes["parent-attribute"] !==
              "undefined"
            ) {
              this.processParentRecord();
            } else {
              //Only on first data from OS as race condition overrides the DS records with old data.
              setTimeout(() => {
                this.processParentRecord();
              }, 500);
            }
    ...
    //setting omni listener attribute if it uses data from OS
    setupOmniListener() {
      let omniDef = this.omniJsonDef;
      if (omniDef?.propSetMap?.customAttributes) {
        omniDef.propSetMap.customAttributes.forEach(attr => {
          let match = attr?.source?.match(/%([^[%]+)%/g);
          if (match && match.length > 0) {
            this._listenOmniAttributes.push(attr);
            this._setupOmniListener = true;
          }
        });
      }
    }
    ...
    processParentRecord() {
      if (
        this._parentAttrbute &&
        (this._parentRecord || this._parentMergeField || this._parentRecords)
      ) {
        Object.keys(this._parentAttrbute).forEach(key => {
          if (this._parentAttrbute[key] === "{record}") {
            this._parent[key] = this._parentRecord;
          } else if (this._parentAttrbute[key] === "{records}") {
            this._parent[key] = this._parentRecords;
          } else {
    ...
    get showLoader() {
      return !this.cardLoaded || this.showActionSpinner;
    }
    ...
    updateDatasource = (event, elementClass) => {
      return new Promise((resolve, reject) => {
        if (event && event.type) event.stopPropagation();
        if (event && event.detail) {
          ...
          this.setDefinition(this.data, {
            ...
            .then(() => {
              ...
              this.getActionDS = false;
              this.actionElementId = null;
              let element = elementClass
                ? this.template.querySelector(elementClass)
                : "";
    ...
    async fireDataAction(event) {
      if (event && event.detail) {
        ...
        const parsedMesasge = replaceAllSpecialChar(message);
        let dataObject = JSON.parse(parsedMesasge);

        //Support to use {records} on updateDS mergefields
        let matched = event.detail.message.match(this._regexPattern);
        if (matched && matched.length > 0) {
          if (
            dataObject?.value?.jsonMap &&
            typeof dataObject.value.jsonMap === "string"
          ) {
            dataObject.value.jsonMap = JSON.parse(dataObject.value.jsonMap);
          }
          ...
          if (
            dataObject?.value?.jsonMap &&
            typeof dataObject.value.jsonMap === "object"
          ) {
            dataObject.value.jsonMap = JSON.stringify(dataObject.value.jsonMap);
          }
        }
        ...
        try {
          ...
          let jsonMap = null;
          if (dataObject?.value?.jsonMap) {
            jsonMap = dataObject.value.jsonMap;
          }
          let dsConfig = dataObject;
          if (dataObject.type === "Query" || dataObject.type === "Search") {
            const OmniUiCardKey = this.data[
              getCardObjectFields(this.data).OmniUiCardKey
            ];
            dsConfig = {
              OmniUiCardKey,
              elementId: this.getActionDS ? this.actionElementId : null,
              jsonMap
            };
          }
          ...
          this.getActionDS = false;
          this.actionElementId = null;
          await this.processDataActionResponse(
            "success",
            response,
            data,
            dataObject,
     ...
     */
    async processDataActionResponse(type, response, data, dataObject, event) {
      if (type === "success") {
        ...
        if (dataObject.type !== "StreamingAPI") {
          await this.recordUpdateOnDataAction(data, event);
        } else {
     ...
     * @param {T} event Event from where it is triggered
     */
    async recordUpdateOnDataAction(data, event) {
      let config = {
        response: data.response,
        node: data.node,
        ignoreResponse: data.ignoreResponse
      ...
      if (data.response && !data.ignoreResponse) {
        try {
          let result = await dataActionUpdateRecords(data);
          config.data = result;
          this._records = [...result];
          this.pickRecords(this._records);
          this.updateKeys();
          await this.setStateObj();
    ...
    triggerReload = (event, elementClass) => {
      return new Promise((resolve, reject) => {
        if (event && event.type) event.stopPropagation();
        this.cardLoaded = false;
        this.eventActionType = "reload";
        this.hidePreviousOpenedModal();
        this.setDefinition(this.data, {
          type: "reload",
          detail: event.detail
        })
    ...
    _contextVariables;
    dataStr;
    async setDefinition(_data, actionObj) {
      //to make sure we are overriding common definition between two instances of same flexcard
      _data = Object.assign({}, _data);
      //backward compatibility fix o nds theme changes
      let existingClass = this.getAttribute("class");
      let checkClassExists =
        existingClass &&
        (existingClass?.indexOf("via-nds") !== -1 ||
          existingClass?.indexOf("via-slds") !== -1);
      this.setAttribute(
        "class",
      ...
      let card = _data;
      this.data = _data;
      this.isFileBased = this.data.Id ? this.isMetaData(this.data.Id) : true;
      ...
      try {
        ...
        if (hasPermission) {
          let param = params(this._currentpageRef);
          param.id = param.id || this.recordId;
          const allParams = this.currentPageReference?.state
            ? { ...param, ...this.currentPageReference.state }
            : param;
          this.Params = allParams;
          this.isPreview =
            this.Params.isPreview === true || this.Params.isPreview === "true";
          ...
          this.updateDebugPanel("Params", { ...this.Params });
          if (this.isPreview) {
            ...
            try {
              const userProfileResult = await this.getCachedUserProfile();
              if (userProfileResult) {
                this.userProfile = { ...userProfileResult };
                this.updateDebugPanel("User", this.userProfile);
              }
            } catch (error) {
          ...
          if (
            ...
            try {
              this.userProfile = (await this.getCachedUserProfile()) || {};
              this.updateDebugPanel("User", this.userProfile);
            } catch (error) {
              console.error("User Profile fetch error : " + error);
            }
          ...
          if (!this.isPreview) {
            if (
              ...
              return Promise.reject("Parent attribute is not available");
            }
          }
          ...
          if (
            ...
            const contextVar = await this.getContextTestVariables();
            this._contextVariables = contextVar;
            this.updateDebugPanel("TestParams", this.testParams || contextVar);
            if (this._parent) {
              this._parent = {
                ...(contextVar.Parent || {}),
                ...this._parent
            ...
            if (this.userProfile) {
              contextVar.User = {
                ...this.userProfile,
                ...(contextVar?.User || {})
              }; //merge context user profile info on preview
            }
            if (this.session) {
              contextVar.Session = {
                ...(contextVar.Session || {}),
      ...
      try {
        ...
        if (
          ...
          if (!hasContext) {
            ...
            await this.setStateObj();
            return "Records are not updated as we don't use any Parent updated value on datasource";
          }
        }
        let response;
        if (
          this.omniSupportKey &&
          !this._preventSaveState &&
          this.eventActionType != "updateDS"
        ) {
    ...
    fetchSelectedCards(newRecords) {
      if (this.selectableMode && this.selectableField && this.card) {
        const cardsList = this.selectedCardsLabel || "selectedcards";
        ...
        if (this.hasRecords) {
          ...
          const selectedCardsArr =
            newRecords && this.card[cardsList] ? this.card[cardsList] : [];
          const recordsArr = newRecords ? newRecords : this._records;
    ...
    fetchDataWithTimeOut(timeoutVal) {
      let timeoutError = new Promise((resolve, reject) => {
        setTimeout(
          reject,
          timeoutVal,
          `Timeout Error : Response timed out at ${timeoutVal} ms`
        );
      });
      let fetchDataRecs = this.getRecords();
      return Promise.race([fetchDataRecs, timeoutError]);
    }

    updateDebugger(currContext, records) {
      currContext.updateDebugPanel("dataSource.records", records);
      if (
        currContext.dsAction &&
        currContext.dsAction.obj &&
        currContext.dsAction.type
      ) {
        currContext.updateActionDebugger(
          currContext.dsAction,
          "Success",
          records
        );
    ...
    async fetchData() {
      try {
        ...
        if (
          ...
          let dsVal = this.data.dataSource.value;
          if (interval) {
            //get data once if interval is set and delay is not
            if (dsVal && !dsVal.dsDelay) {
              try {
                const records = await this.getRecords();
                this.updateDebugger(this, records);
              } catch (e) {
                this.handleDataSourceError(e);
                return Promise.reject("Error in fetching records");
              }
            }
            // eslint-disable-next-line @lwc/lwc/no-async-operation
            this.cardInterval = setInterval(async () => {
              try {
                const records = await this.getRecords();
                this.updateDebugger(this, records);
              } catch (e) {
          ...
          if (dsVal && (dsVal.timeout || dsVal.dsDelay)) {
            let records = [];
            let timeoutVal = parseInt(dsVal.timeout);
            let delayVal = dsVal.dsDelay !== null ? parseInt(dsVal.dsDelay) : 0;
            try {
              //Delay is the time the user adds to wait before the DS call is made.
              if (delayVal > 0) {
                await new Promise(resolve => {
                  setTimeout(resolve, delayVal);
                });
              }
              //fetch timeout is to cancel the DS call to avoid long wait times
              if (timeoutVal > 0) {
                records = await this.fetchDataWithTimeOut(timeoutVal);
              } else {
              ...
              this.hasTimeoutError =
                typeof e === "string" && e.indexOf("Timeout Error") !== -1;
              this.cardLoaded = true;
              this.hasError = true;
    ...
    getRecords() {
      return new Promise((resolve, reject) => {
        ...
        const OmniUiCardKey = this.isFileBased
          ? this.data.uniqueKey || this.data.Name
        ...
        if (this.data.dataSource.type === "StreamingAPI") {
          ...
          loadSDK(this, this.savedConfiguration)
            .then(sdkInstance => {
              if (sdkInstance && !this.hasTimeoutError) {
                invokeSDKMethod(this.data.dataSource, sdkInstance)
                  .then(sdkResponse => {
                    if (!this.hasTimeoutError) {
                      this._records =
                        typeof sdkResponse === "string"
                          ? JSON.parse(sdkResponse)
                          : sdkResponse;
                      this.isRecordLoaded = true;
                      this._recordProcessed = false;
                      this.convertRecordsToArray();
          ...
          const elementId = this.getActionDS ? this.actionElementId : null;
          doEncryptedDatasourceFlex(
            OmniUiCardKey,
            jsonMap,
            elementId,
        ...
        } else if (this.data.dataSource.type === "ApexRest" && this.isPreview) {
          getCardDataById(this.card?.Id).then(resp => {
            const fields = getCardObjectFields(resp);
            const sampleDataSourceResponse =
              typeof resp[fields.SampleDataSourceResponse] === "string"
                ? JSON.parse(resp[fields.SampleDataSourceResponse])
                : resp[fields.SampleDataSourceResponse];
            this._records = sampleDataSourceResponse;
    ...
    updateAppendedRecordsKey(newRecords, i) {
      const updatedRecords = [];
      newRecords.forEach(record => {
        if (record) {
          const cardRecord = this.updateRecordWithUniqueKey(record, i);
          cardRecord.Id = record.Id || "REC" + i;
          updatedRecords.push(cardRecord);
        }
        i++;
      });
    ...
    updateKeys() {
      if (this.records) {
        if (Array.isArray(this.records) && !this.hasTimeoutError) {
          const cardRecord = [];
          this._records.forEach((record, i) => {
            if (record) {
              const obj = this.updateRecordWithUniqueKey(record, i);
              if (!obj.Id) {
                obj.Id = obj.uniqueKey;
              }
              cardRecord.push(obj);
            }
          });
    ...
    updateRecordWithUniqueKey(record, i) {
      const uniqueVal =
    ...
    handleCardStyleUpdate = (event, elements) => {
      ...
      Object.keys(this.cardStyles).forEach(key => {
        if (stateElement.querySelector('[data-style-id="' + key + '"]')) {
          ...
          if (defaultStyle) {
            ...
            // TODO: W-10802478 and W-10980897 Need to change it to be the same irrespective of element type.
            if (element.tagName.toLowerCase().includes("menu-item")) {
      ...
      if (this.data && this.data.states) {
        ...
        if (elements && elements.length > 0) {
          ...
          this.data.states.forEach(item => {
            ...
            } else if (!item.blankCardState && this.hasRecords) {
              this.hasActiveState = true;
            }
          });
          this.selectState(elements);
    ...
    setStateOfAppendedRecords(rindex, singleRecord) {
      if (this.data?.states) {
        const elements = this.template.querySelectorAll(`.cf-vlocity-state`);
        const newElements = Array.from(elements).filter(e =>
          singleRecord
            ? parseInt(e.dataset.rindex, 10) === rindex
            : parseInt(e.dataset.rindex, 10) >= rindex
        );
        if (newElements && newElements.length > 0) {
          this.selectState(newElements);
          this.createCardsArray(elements); // To update the Data JSON panel in designer preview
          this.handleCardStyleUpdate(null, newElements);
        }
      }
    }

    selectState(elements) {
      ...
      elements.forEach(element => {
        ...
        const stateIndex = parseInt(element.dataset.index, 10);
        this._allMergeFields.Flex.index = element.dataset?.rindex || 0;
        const state = this.data.states[stateIndex];
        const obj = element.record ? element.record : {};
        ...
        if (!selectedState) {
          ...
          if (satisfyCondition) {
            if (state.blankCardState && this.hasActiveState) {
              // when we have Active state blankstate should be always false
              selectedState = false;
            } else if (state.blankCardState && !this.hasActiveState) {
          ...
          const sessionInter = interpolateWithRegex(
            this.session,
            {
              Params: this.Params,
              obj
          ...
          if (this.orgNsPrefix) {
            element.orgNsPrefix = this.orgNsPrefix;
          }
    ...
    async getContextTestVariables() {
      const result = this.isFileBased
        ? await getflexCardMetadata({ uniqueName: this.data.uniqueKey })
        : await getCardDataById(this.data.Id);
      const cardObjFields = this.isFileBased
        ? getCardObjectFields(this.data, true)
        : getCardObjectFields(this.data);
      const datasourceData = result[cardObjFields.PropertySetConfig]
        ? JSON.parse(result[cardObjFields.PropertySetConfig]).dataSource
        : null;
      let contextVar = {};
    ...
    appendRecord(data) {
      if (this.hasLayoutRecord && this._dataNode === "{records}") {
        // If it is a Child Card with datanode records then update parent records array
        const evtData = {
          data,
          type: "appendRecord"
        };
        this.fireEvent("updateparent", evtData);
        return;
      }
      let val = data.detail.value;
      val = typeof val === "string" ? JSON.parse(val) : val;
      if (!this._records) {
        this.showToast(AppendRecordNoRecordsError, ErrorToastTitle, "error");
        const actionEvtObj = getActionDebugEventObj(
          "Append Record",
          val,
          null,
          data.detail.key
      ...
      let updatedRecords;
      let isRecordsUpdate = false;
      const singleRecord = data.detail.key !== undefined;
      let recordIndex = this._records.length;
      if (val && Array.isArray(val)) {
        if (data.detail.appendToRecords) {
          updatedRecords = this.updateAppendedRecordsKey(val, recordIndex);
          this.fetchSelectedCards(updatedRecords);
          const recordData = [...this._records];
          recordData.push(...updatedRecords);
          this._records = [...recordData];
          isRecordsUpdate = true;
        } else if (data.detail.key) {
          const index = this.getStateIndex(data);
          recordIndex = index;
          const obj = this._records[index];
          if (obj) {
            const recordData = get(obj, data.detail.key);
            if (recordData && Array.isArray(recordData)) {
              recordData.push(...val);
              set(obj, data.detail.key, recordData);
              this._records[index] = { ...obj };
              isRecordsUpdate = true;
            }
          }
        }
        if (isRecordsUpdate) {
          Promise.resolve().then(() => {
            this.setStateOfAppendedRecords(recordIndex, singleRecord);
          });
          const actionEvtObj = getActionDebugEventObj(
            "Append Record",
            val,
        ...
        this.showToast(AppendRecordTypeError, ErrorToastTitle, "error");
        const actionEvtObj = getActionDebugEventObj(
          "Append Record",
          val,
          this._records,
        ...
        this.updateActionDebugger(actionEvtObj, "Error", AppendRecordTypeError);
      }
    }
    ...
    rerenderState(fields, index, selectSingleCard, data) {
      ...
      if (this.hasLayoutRecord && this._dataNode === "{records}") {
        ...
        if (setValueObj.parentData) {
          ...
          this.fireEvent("updateparent", setValueObj.parentData);
        }
      } else {
        try {
          setValueObj = {
          ...
          this.showToast(error, ErrorToastTitle, "error");
        }
      }
      if (setValueObj) {
    ...
    resetSelectedCards = event => {
      if (event && event.type) event.stopPropagation();
      if (
        this.selectedCardsLabel &&
        this[this.selectedCardsLabel]?.length &&
        this.records?.length
      ) {
        this[this.selectedCardsLabel] = [];
        let fieldName = this.selectableField
          ? this.selectableField
          : "Flex.isSelected";
        this._records = this._records.map(record => {
          if (record) {
            if (fieldName === "Flex.isSelected") {
              record._flex = record._flex ? { ...record._flex } : {};
              record._flex.isSelected = false;
              record.Flex = record.Flex ? { ...record.Flex } : {};
              record.Flex.isSelected = false;
            } else {
        ...
        if (this.hasLayoutRecord && this._dataNode === "{records}") {
          this.fireEvent("updateparent", { records: this._records });
        } else {
          this.pickRecords(this._records);
          this.updateKeys();
          this.setStateObj();
        ...
        cardObj[this.selectedCardsLabel] = [];
        this.card = { ...cardObj };
        this._allMergeFields = extractCardMergeFields(this.card);
      }
    };
    ...
    updateParentData(event) {
      ...
      if (eventDetail.type === "appendRecord" && eventDetail.data) {
        this.appendRecord(eventDetail.data);
      } else if (
        (eventDetail.record && eventDetail.index) ||
        eventDetail.records
      ) {
      ...
      return new Promise((resolve, reject) => {
        if (this.pendingCommunityPromise) {
          this.pendingCommunityPromise
            .then(() => {
              resolve(this.isCommunity ? getCommunityPrefix() : "");
            })
            .catch(e => {
    ...
    getActionSpinnerValue(showSpinner, recordIndex) {
      //Accepted value => true, false, "true", "false", "True", "False" e.t.c
      let value = showSpinner;
      if (typeof showSpinner === "string") {
        const record = Array.isArray(this.records)
          ? this.records[recordIndex] || {}
          : {};
        value = this.interpolateValue(showSpinner, record);
      }
      value = typeof value === "string" ? value.toLowerCase() : value;
      return value === "true" || value === true;
    }

    provideOldEventListenerFields(eventObj) {
      //Below fields are not present in 234 release, hence this method will initialize,
      // if they are not present for older generated cards
      if (!eventObj.actionElementClass) {
    ...
    multipleEventActionQueue = [];
    isMultipleActionRunning = false;

    executeWaitingMultipleAction() {
      if (this.multipleEventActionQueue.length > 0) {
        const firstEvent = this.multipleEventActionQueue.shift();
        const {
          eventObj,
          actionIndex,
          previousResponse,
        ...
        this.showActionSpinner = this.getActionSpinnerValue(
          eventObj.showSpinner,
          recordIndex
        );
    ...
    fireMultipleActionRecursively(
      ...
      eventObj = this.provideOldEventListenerFields(eventObj);
      if (eventObj.eventSource !== "EventListener") {
        this.isMultipleActionRunning = false;
      }
      if (this.isMultipleActionRunning) {
        this.multipleEventActionQueue.push({
          eventObj,
          actionIndex,
          previousResponse,
          eventIndex,
    ...
    fireEvtListenerActions(
      ...
      if (actionIndex < eventObj.actionList.length) {
        this.isMultipleActionRunning = true;
        eventObj.actionData = eventObj.actionList[actionIndex];
        const actionEventParams =
          eventObj.eventtype === "event" ? customEvent.detail : customEvent;
        ...
        if (fireAction) {
          ...
          const actionElement = eventObj.actionElement
            ? eventObj.actionElement
            : this.template.querySelector(eventObj.actionElementClass);
          if (actionElement) {
            actionElement
              .executeAction(
                actionCallerObj,
              ...
              .then(result => {
                if (
                  result &&
                  (result.actionType === "DataAction" ||
                    result.actionType === "cardAction" ||
                    result.actionType === "updateOmniScript")
                ) {
                  const actionEleId = eventObj.elementId
                    ? `${eventObj.elementId}_${actionIndex}`
                    : null;
                  this.fireCardAction(
                    result.subType,
                    result,
                    eventObj.actionElementClass,
                    actionEleId
                  )
                    .then(response => {
                      this.fireEvtListenerActions(
        ...
        this.showActionSpinner = false;
        this.isMultipleActionRunning = false;
        this.executeWaitingMultipleAction();
      }
    }

    handleEventListenerError(error) {
      let errMsg = typeof error !== "string" ? error?.toString() : error;
      console.error("Action Errored", errMsg);
      ...
      this.showToast(errMsg, ErrorToastTitle, "error");
      this.updateActionDebugger(errorEvent, "Error", errMsg);
      this.showActionSpinner = false;
      this.isMultipleActionRunning = false;
      this.executeWaitingMultipleAction();
    }
    ...
    createActionCallerObject(
      ...
      if (eventObj.eventSource === "EventListener") {
        ...
        if (
          (this.card.isRepeatable === true ||
            typeof this.card.isRepeatable === "undefined") &&
          typeof eventObj.recordIndex !== "undefined"
        ) {
      ...
      const actionData = { action: customEvent };
      const sessionVar = {};
      ...
      if (
        ...
        if (eventObj.eventSource === "EventListener") {
          ...
          this.record = { ...actionData };
        }
      }
      return actionCallerObj;
    }

    async fireCardAction(type, data, elementClass, elementId) {
      let obj;
      try {
        switch (type) {
          case "reload":
            await this.triggerReload(data, elementClass);
            break;
          case "remove":
            await this.remove(data);
            break;
          case "updatedatasource":
            this.getActionDS = true;
            this.actionElementId = elementId;
            await this.updateDatasource(data, elementClass);
            break;
          case "setValues":
            obj = { detail: data?.actionDetails };
            this.updateAction(obj);
            break;
          case "selectcards":
            obj = { detail: data };
            this.updateSelectedCards(obj);
            break;
          case "Data Action":
            this.getActionDS = true;
            this.actionElementId = elementId;
            await this.fireDataAction(data);
            break;
          case "updateOmniScript":
            this.updateOmniscript(data);
            break;
          case "appendRecord":
    ...
    setActionInstance(eventObj, actionEventParams, eventIndex, actionIndex) {
      const actionType = eventObj?.actionData?.stateAction?.type;
      const openFlyoutIn = eventObj?.actionData?.stateAction?.openFlyoutIn;
      if (actionType === "Flyout") {
        this.hidePreviousOpenedModal();
        this.action = {};
      }
      let currentFlyoutIndex = "";
      if (actionEventParams) {
        this.action = { ...this.action, ...actionEventParams };
      }
      if (eventObj.eventSource === "EventListener") {
        currentFlyoutIndex = "listener" + eventIndex + "action" + actionIndex;
        this.action[currentFlyoutIndex] = actionEventParams || true;
      } else if (eventObj.eventSource === "EventTrigger") {
        currentFlyoutIndex = eventObj.elementLabel + "action" + actionIndex;
        this.action[currentFlyoutIndex] = true;
      } else if (eventObj.eventSource === "EventFlexAction") {
        //Menu Item or Direct Flex Action
        currentFlyoutIndex = eventObj.elementLabel + "action" + actionIndex;
        if (actionType === "Flyout") {
          if (openFlyoutIn === "Modal") {
            this.modalAction[currentFlyoutIndex] = true;
          } else if (openFlyoutIn === "Popover") {
            this.popoverAction[currentFlyoutIndex] = true;
          }
          this.hideFlyoutInSameActionSeq(eventObj, currentFlyoutIndex);
        }
      }
    }

    hideFlyoutInSameActionSeq(eventObj, currentFlyoutIndex) {
      this.currentFlyoutIndex = currentFlyoutIndex;
      const currentFlexActionEl = eventObj.actionElement
        ? eventObj.actionElement
        : this.template.querySelector(eventObj.actionElementClass);
      if (!this.HideActionInSameSeq) {
        this.HideActionInSameSeq = delay(0).then(() => {
          this.HideActionInSameSeq = false;
          const allSlots = currentFlexActionEl?.childNodes;
          if (allSlots && allSlots.length > 0) {
            allSlots.forEach(slotEl => {
              const theme = (this.card && this.card.theme) || "slds";
              if (slotEl.classList.contains(this.currentFlyoutIndex)) {
                slotEl.classList.remove(`${theme}-hide`);
              } else {
                slotEl.classList.add(`${theme}-hide`);
              }
            });
          }
        });
    ...
    hidePreviousOpenedModal() {
      const actionTrigger = this.template.querySelector(".action-trigger");
      const executeAction = this.template.querySelector(".execute-action");
      const flexActionEl = this.template.querySelectorAll(".flexActionElement");
      actionTrigger?.closeFlyout();
      executeAction?.closeFlyout();
      //For Menu and Direct Flex Action Elements
      if (flexActionEl) {
        flexActionEl.forEach(flexAction => {
          flexAction?.closeFlyout();
        });
      }
    }

    /**
     * Triggers whenever an action on flexcard is executed using action element
     * @param {*} event
     */
    performAction(event) {
      ...
      if (
        ...
        stateIndex = event.currentTarget.closest(".cf-vlocity-state").dataset
          .index;
        //This will be undefined in case of repeat record is off
        recordIndex = event.currentTarget.closest(".cf-vlocity-state").dataset
          .rindex;
      }
      ...
      if (actionList?.length) {
        const eventObj = {
          ...
          elementId: dataset.actionKey,
          actionElement: event.currentTarget
        };
    ...
    /**
     * Triggers when an action executed from other elements that supports the same, eg: block/img
     * @param {*} event
     */
    executeAction(event) {
      const dataset = event.currentTarget.dataset;
    ...
    /**
     * Card event listener's get handle here
     * @param {*} eventObj
     * @param {*} eventIndex
     * @param {*} event
     */
    handleEventAction(eventObj, eventIndex, event) {
      eventObj.actionList =
        eventObj.actionList ||
        (eventObj.actionData ? [eventObj.actionData] : []);
      eventObj.eventSource = "EventListener";
      eventObj.actionElementClass = ".action-trigger";
      eventObj.elementId = eventObj.key;
      let recordIndex;
    ...
    /**
     * Method fetches custom label.
     */
    fetchUpdatedCustomLabels() {
      fetchCustomLabels(Object.keys(this.Label), null, null, true).then(
        labels => {
          this.Label = labels;
          let card = { ...this.card };
          card.Label = labels;
    ...
    /**
     * Loads custom global css stylesheet attachment
     * @param {*} customCssAttachmentId
     */
    loadCustomStylesheetAttachement(customCssAttachmentId) {
      this.getCommunityPrefix().then(result => {
        loadStyle(
          this,
          result + `/servlet/servlet.FileDownload?file=${customCssAttachmentId}`
        ).then(() => {
          //Loading Custom CSS.
        });
      });
    ...
    /**
     * Wire record change event handler
     * @param {*} error
     * @param {*} response
     * @param {*} index
     */
    recordChangeEventHandler(error, response, index) {
      if (error) {
        let message = "";
        ...
        this.showToast(message, "Record Change Listener Error", "error");
      } else if (response) {
        this.handleEventAction(this.data.events[index], index);
      }
    }

    /**
     * Get optional fields while setting up record change listener
     * @param {*} eve
     * @returns []
     */
    getWireOptionalFields(eve) {
      if (eve.optionalFields.indexOf("{") !== -1) {
        return this._allMergeFields
          ? interpolateWithRegex(
              eve.optionalFields,
              this._allMergeFields,
              this._regexPattern,
        ...
        return eve.optionalFields.split(",");
      }
    }
  };
