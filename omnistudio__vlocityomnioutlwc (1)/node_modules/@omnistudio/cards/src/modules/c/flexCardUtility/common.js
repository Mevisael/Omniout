...
function extractCustomLabelUsed(obj, label) {
  ...
  let labelWithNsPrefixArr = {};
  matchedArray.forEach(labelString => {
    let labelArr = labelString.split(".");
    let customLabel;
    //Handling three types of usecase
    //a) {Label.<customLabel>} b) {Label.<namespacePrefix>__<customLabel>} c) {Label.<StandardLabelSection>.<StandardLabel>}
    if (labelArr.length === 2) {
      //Usecase (a) & (b)
      customLabel = labelArr[1] && labelArr[1].replace(/(}|%7D)/g, "");
    } else if (labelArr.length === 3) {
      //Usecase (c)
      customLabel = `${labelArr[1]}.${labelArr[2] &&
        labelArr[2].replace(/(}|%7D)/g, "")}`;
      labelWithNsPrefixArr[labelArr[2].replace(/(}|%7D)/g, "")] = labelArr[1];
    }
  ...
  //eg: customLabelObj = {"Account":null,"OmnistudioRuntime.Test":null}
  if (isNewLabel) {
    return fetchCustomLabels(Object.keys(customLabelObj)).then(labels => {
      let copiedLabels = { ...labels };
      Object.keys(labelWithNsPrefixArr).forEach(item => {
        let labelVal = copiedLabels[`${labelWithNsPrefixArr[item]}.${item}`];
        delete copiedLabels[`${labelWithNsPrefixArr[item]}.${item}`];
        set(copiedLabels, `${labelWithNsPrefixArr[item]}.${item}`, labelVal);
      });
      return copiedLabels;
    });
  }
