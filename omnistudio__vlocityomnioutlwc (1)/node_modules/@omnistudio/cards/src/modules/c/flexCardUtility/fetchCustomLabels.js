 ...
 * Build: v234.0.0
 */
import { getCustomLabels, getStandardLabels } from "c/flexCardUtils";
...
let fetchUserLanguage = function(multiLanguage) {
  return new Promise(function(resolve) {
    if (activeLanguageCode) {
      resolve(activeLanguageCode);
    } else {
      locale = multiLanguage
        ? null
        : locale || localStoreMethods.getItem("userLocale");
...
let fetchLabels = (labelsArray, returnedLabels, defaultValuesObj, language) => {
  return new Promise(function(resolve, reject) {
    //divide standard and non standard custom labels to make respective calls
    let standardLabels = [];
    let nonStandardLabels = [];

    labelsArray.forEach(item => {
      if (item.split(".").length === 2 && standardLabels.indexOf(item) === -1) {
        standardLabels.push(item);
      } else if (nonStandardLabels.indexOf(item) === -1) {
        nonStandardLabels.push(item);
      }
    });
    getCustomLabel(nonStandardLabels, language)
      .then(res => {
        ...
        //Checking if there are any standard labels used otherwise ignoring this call
        if (standardLabels.length > 0) {
          getStandardLabels({ labelNames: standardLabels }).then(
            resolvedStandardLabels => {
              if (resolvedStandardLabels) {
                Object.keys(resolvedStandardLabels).forEach(key => {
                  returnedLabels[key] = resolvedStandardLabels[key];
                  labelMap[key] = { [language]: resolvedStandardLabels[key] };
                });
              }
...
let fetchCustomLabels = (
  ...
  return new Promise(function(resolve, reject) {
    fetchUserLanguage(multiLanguage).then(function(lang) {
