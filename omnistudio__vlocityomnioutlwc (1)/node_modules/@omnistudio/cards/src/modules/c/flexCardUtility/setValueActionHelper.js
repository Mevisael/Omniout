 ...
 * Build: v234.0.0
 */
import { flexInvalidIndex } from "c/flexCardUtils";
let card;
let fields;
  ...
  if (fields) {
    fields.forEach(item => {
      ...
      switch (key) {
        ...
        case "record":
          if (records) {
            recordObj = { ...validObj(fieldVal) };
            records[index] = recordObj;
            response.records = [...records];
          }
          break;
        default:
          ...
          if (mergeFieldKey) {
            ...
            if (
              item.fieldName === "_flex.isSelected" ||
              item.fieldName === "Flex.isSelected"
            ) {
              recordObj._flex = recordObj._flex ? { ...recordObj._flex } : {};
              recordObj._flex.isSelected = fieldVal;
              recordObj.Flex = recordObj.Flex ? { ...recordObj.Flex } : {};
              recordObj.Flex.isSelected = fieldVal;
            } else {
