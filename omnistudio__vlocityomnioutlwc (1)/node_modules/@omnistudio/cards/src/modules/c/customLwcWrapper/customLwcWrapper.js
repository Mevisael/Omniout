...
export default class customLwcWrapper extends BaseFlexElementMixin(
  ...
  set record(val) {
    ...
    if (this.isElementRendered) {
      ...
      this.setAttributes(this.unInterpolatedProperty, true);
    }
  ...
  set property(val) {
    this._property = val
      ...
      : {};
    if (val && !isEmpty(val)) {
      this.isOmniscript = this._property?.prefill !== undefined;
      this.unInterpolatedProperty = { ...this._property };
  ...
  renderedCallback() {
    let ele =
      this.querySelector(this.elementName) ||
      this.querySelector(".wrapperElement");
  ...
  interpolatedVal(property) {
    let data = property ? property : this.unInterpolatedProperty;
    let obj =
  ...
  interpolateAttributes(propertyObj, obj) {
    if (propertyObj && propertyObj.prefill) {
      ...
      const apiVars = ["{record}", "{records}", "{recordId}"];
      for (const key in attributes) {
        if (key) {
          const value = attributes[key];
          if (apiVars.indexOf(value) !== -1) {
            const apiVar = value.replace(/[{}]/g, "");
            if (this[apiVar]) {
              attributes[key] = this[apiVar];
            }
          }
        }
      }
  ...
  interpolateOS() {
    const obj = { ...this.record, ...this._allMergeFields };
    if (this.action) {
      obj.action = this.action;
    }
    this.interpolatedProperty = this.interpolateAttributes(
      this.unInterpolatedProperty,
      obj
    );
    this._property = { ...this.interpolatedProperty };
