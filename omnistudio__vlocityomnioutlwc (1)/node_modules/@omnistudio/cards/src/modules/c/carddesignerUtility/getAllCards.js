...
export function getAllCards() {
  return getAllFlexCards().then(cards => {
...
export function getAllChildCards() {
  ...
  allCardsByRecordType = getAllCardsByRecordType("Flex").then(cards => {
      ...
      cards.forEach(card => {
        ...
        if (
          ...
          if (card[cardObjectFields.PropertySetConfig]) {
            ...
            if (Array.isArray(cardDef.states) && cardDef.states.length) {
    ...
    allChildCardsLoaded = true;
    return childCards;
  });
  return allCardsByRecordType;
}
export async function getAllCardsDetailsByRecordType(
  recordType,
  scope,
  filters
) {
  const bpoCards = await getAllCardsDetailsByRecordTypeMethod({
    recordType: recordType,
    scope: scope,
    filters: filters
  });
  const fileBaseCards = await getFileBasedFlexCards();
  const allCards = bpoCards ? [...bpoCards] : [];
  if (fileBaseCards?.length) {
    fileBaseCards.forEach(card => {
      card.IsFileBased = true;
      allCards.push(card);
    });
  }
  const transformedElements = [];
  allCards.forEach(elementPreConvert => {
    let element = Object.assign({}, elementPreConvert);
    transformedElements.push(element);
  });
  return Object.assign([], transformedElements);
}
export function isChildCardLoaded() {
  return allChildCardsLoaded;
}
