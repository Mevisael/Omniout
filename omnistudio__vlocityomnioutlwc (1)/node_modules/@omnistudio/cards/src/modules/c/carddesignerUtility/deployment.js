...
import getAllLWCBundle from "@salesforce/apex/FlexCardController.getAllLWCBundle";

const apiVersion = "v56.0";

const urlPrefix = `/services/data/${apiVersion}/tooling/`,
...
let javaDeploymentInProgress = false;
...
function getAllBundles(nextRecordsUrl) {
  return new Promise((resolve, reject) => {
    getAllLWCBundle()
      ...
      .then(res => {
        if (res?.length) {
          lwcBundles = lwcBundles.concat(res);
          resolve(lwcBundles);
        }
      })
      .catch(error => {
...
function updateCardResourceWithJava(cardObj) {
  if (!javaDeploymentInProgress) {
    let cardObjectFields = getCardObjectFields(cardObj);
    let isActive = cardObj[cardObjectFields.IsActive];
    let nameOfLwc = isActive
      ? cardObj.Name
    ...
    let updatedResources = checkIfChangeInSource(cardObj);
    if (updatedResources.length > 0) {
      javaDeploymentInProgress = true;
      return new Promise((resolve, reject) => {
        lwcPartialUpdate(nameOfLwc, updatedResources, cardObj, "partialUpdate")
          .then(res => {
            if (!res?.hasErrors) {
              //When we create new file for styleDefinition we need to update its Id in local resource
              //it will always be at 0th index as adding its resource request using unshift.
            ...
            updateLocalResource(nameOfLwc, updatedResources);
            javaDeploymentInProgress = false;
            resolve(true);
            // eslint-disable-next-line no-console
            console.log(res);
          })
...
function checkIfChangeInSource(cardObj) {
  ...
  let metaObj = getMetaData(nameOfLwc, cardObj, "card");
  let files = [...metaObj?.lwcResources?.lwcResource];
  if (isActive) {
    ...
    if (
      updatedXml &&
      fileExist &&
      updatedXml.replace(/\s+/g, "").trim() !==
        fileExist.Source.replace(/\s+/g, "").trim()
    ) {
      // Replace the files xml with the updated one
      const xmlIndex = findIndex(files, {
        filepath: xmlPath
      });
      files.splice(xmlIndex, 1, {
        name: xmlPath,
        source: updatedXml,
        filepath: xmlPath
      });
  ...
  files.forEach(file => {
    if (
      ...
      if (cardExists && file.source) {
        if (
          file.source.replace(/\s+/g, "").trim() !==
          cardExists.Source.replace(/\s+/g, "").trim()
        ) {
        ...
        // If the package.xml file is not part of the local resources, then don't add it as part of the updatedResources
        file?.filepath !== "package.xml" &&
...
let getVfDomain = () => {
  let cachedVfBaseUrl = localStoreMethods.getItem("visualforceDomain");
  if (!cachedVfBaseUrl) {
    getVisualforceDomain().then(vfBaseUrl => {
      localStoreMethods.setItem("visualforceDomain", vfBaseUrl);
    });
  }
};
...
function isRemoteSiteSetting() {
  return new Promise((resolve, reject) => {
    ...
    let lightningBaseUrl =
      ...
      window.location.hostname +
      (window.location.port ? ":" + window.location.port : "");
    localStoreMethods.setItem("lightningBaseUrl", lightningBaseUrl);
