 ...
 * Build: v234.0.0
 */
import { convertNameToValidLWCCase, isStandardRuntime } from "./commonUtil";
import {
  namespace,
  isInsidePckgPromise,
...
const BACK_COMPAT_STANDARD_RUNTIME_MODE = "backcompatstandardruntime";
let downloadMode = "";
...
function getActionType(ele) {
  let type =
    isSingleAction(ele) && ele.property.stateAction
...
function generateElementDiv(element, theme, type, omniSupport, item) {
  ...
  if (ele.element === "childCardPreview") {
    ...
    if (ele.property?.customLwcData) {
      ...
      selectedCustomLwc = find(allLwcBundles, { DeveloperName: ele.element });
    }
    ...
    elementName = lwcPropertyNameConversion(cmpName);
  }
    ...
    if (actionWithEvents(ele)) {
      ...
      if (isSingleAction(ele)) {
        ...
        if (!ele.property.actionList) {
          ele.property.actionList = ele.property?.stateAction
            ? [ele.property]
            : [];
        }
        ...
        data-element-label="${getElementLabel(ele)}"  ${generateElementProperty(
          ele
        )} theme="${theme}" ${actionEnabledHtml} ${preloadingConditionalElement(
          ele,
          renderedElementCount
        )}>${generateFlexActionTemplate(ele)}</${elementName}>
      </div>`;
      } else if (
...
function getFlexActionFields(menuItem) {
  const actionList =
    menuItem.actionList || (menuItem.actionData ? [menuItem.actionData] : []);
  let firstFlyoutAction = null;
...
function generateFlexMenuItems(
  menuItemsArray,
  theme,
  renderedElementCount,
  flexMenuElementLabel
) {
  let menuItems = ``;
  menuItemsArray.forEach((menuItem, index) => {
    ...
    if (!menuItem.actionConditions) {
      ...
      menuItem.actionConditions =
        menuItem?.actionData?.stateAction?.actionConditions;
    ...
    >
    <${nsPrefix}-flex-action record={record} card={card} ${generateElementProperty(
      flexActionFields
    )} class="${renderedElementCountMenuItem} flexActionElement" onexecuteaction={performAction} 
...
function getActionEnabledProperties(ele, theme) {
  ...
  const actionList =
    ele.property.action.actionList ||
    (ele.property.action.actionData ? [ele.property.action.actionData] : []);
  ...
  if (actionList && actionList.length > 0) {
    actionList.forEach((action, actionIndex) => {
      ...
      actionElementsSlotObject[
        actionFlyoutIndex
      ] = generateFlyoutForTriggerActionElement(action, actionFlyoutIndex);
    });
  }
  let actionEnabledHtml = `${
    ele.property.action.eventType ? ele.property.action.eventType : "onclick"
  }={executeAction} data-element-label="${elementLabel}" data-action-key="${renderedElementCount}"`;

  // Adding tabindex and onkeydown event only to the flexImg to minimize the impact of the change
  if (
    (ele.element === "flexImg" || ele.element === "block") &&
    actionEnabledHtml &&
    actionEnabledHtml.includes("onclick")
  ) {
...
function generateFlyoutForTriggerActionElement(
  action,
  index,
  actionComponent,
  isSingleFlyout
  ...
  if (stateAction.type === "Flyout" && stateAction.flyoutType) {
    ...
    let defaultNs = selectedCustomLwc
      ? (isInsidePackge &&
          ...
          selectedCustomLwc.NamespacePrefix) ||
        "c"
      : null; //generate OS and Childcard will have namespace as c
    if (isInsidePackge && downloadMode === DOWNLOAD_OFF_PLATFORM_MODE) {
      defaultNs = "c";
    }
    let slotName = "flyout";
    ...
    return `<div slot="${slotName}" class="${index} ${
      stateAction.flyoutContainerClass ? stateAction.flyoutContainerClass : ""
    }" ${condition}>${createFlyoutTemplate(
      "",
      stateAction,
      customLwcElementName,
      defaultNs,
      `{${slotControlVar}.${index}}`
    )}</div>`;
  }
  return "";
}
...
function generateFlyoutActionElement(element) {
  let action = isSingleAction(element)
    ? { ...element.property.stateAction }
    : {};
  let selectedCustomLwc = {};
...
function createFlyoutTemplate(
  ...
  let elementName = lwcPropertyNameConversion(cmpName);
  let template = "";
  ...
  let propertyValue = action.flyoutParams
    ...
    : "";
  if (isOsFlyout) {
    let prefillObj = { omniscriptEmbeddedSource: "flyout" };

    if (action.flyoutParams) {
      prefillObj = { ...prefillObj, ...action.flyoutParams };
    }
...
const FLEXCARD_OS_SUPPORT_PROPS = [
  "omniSupportKey",
  "omniScriptHeaderDef",
  "omniResume",
  "omniSeedJson",
...
export default class ${lwcComponentName} extends LightningElement {
    ...
    ${
      cardDef?.sessionVars
        ? cardDef?.sessionVars
            ...
            .map(session =>
              makeStandardRuntimeProperty(exposedAPIPrefix + session.name)
            )
            .join("\n\n")
        : ""
    }
    ${
      cardDef?.osSupport
        ? FLEXCARD_OS_SUPPORT_PROPS.map(prop => `@api ${prop};`).join("\n\t")
        : ""
    }
    ${
      cardDef?.osSupport
...
function generateStandardRuntimeWrapperHtml(flexCardName, cardDef, nsPrefix) {
  return `<template>
    <${nsPrefix}-flex-card-standard-runtime-wrapper flexcard-name="${flexCardName}"
      ${FLEXCARD_COMMON_PROPS.map(
        prop => `${lwcPropertyNameConversion(prop)}={${prop}}`
      ).join("\n\t")}
      ${
        cardDef?.osSupport
          ? FLEXCARD_OS_SUPPORT_PROPS.map(
              prop => `${lwcPropertyNameConversion(prop)}={${prop}}`
            ).join("\n\t")
          : ""
      }>
    ...
    cardDef.events.forEach((eve, index) => {
      ...
      if (eve.eventtype === "pubsub") {
        ...
        groupEvents[channelName].push(
          `[interpolateWithRegex(\`${eve.eventname}\`,this._allMergeFields,this._regexPattern,"noparse")]: this.handleEventAction.bind(this, data.events[${index}],${index})`
        );
...
function generateDefinition(object) {
  ...
  const uniqueKey = object.UniqueName
    ? object.UniqueName
    : object.IsActive
    ? object.Name
    : `${object.Name}_${object.VersionNumber}_${object.AuthorName}`;
...
function generateJsMetaXML(lwcComponentName, cardObj, metaObject) {
  const cardObjectFields = getCardObjectFields(cardObj);
  const isActive = cardObj[cardObjectFields.IsActive];
  const isChildCard =
    cardObj[cardObjectFields.IsChildCard] ||
    (cardObj[cardObjectFields.Type] &&
      cardObj[cardObjectFields.Type].toLowerCase() === "child");
  let xmlStr = `<?xml version="1.0" encoding="UTF-8"?>
  <LightningComponentBundle xmlns="http://soap.sforce.com/2006/04/metadata">
    <apiVersion>${
      isActive
        ? metaObject?.apiVersion || defaultXmlConfig.api
        : defaultXmlConfig.api
    }</apiVersion>
    <isExposed>true</isExposed>
    <masterLabel>${
      isActive ? metaObject?.masterLabel || cardObj.Name : lwcComponentName
    }</masterLabel>
    `;
  ...
  if (metaObject?.targetConfigs) {
    ...
    targetElements.map(target => {
      if (target.textContent === "lightning__AppPage")
        targetConfig += defaultXmlConfig.appPageConfig;
      if (target.textContent === "lightning__RecordPage")
        targetConfig += defaultXmlConfig.recordPageConfig;
    });
    if (targetConfig) {
      targetConfigsStr = `<targetConfigs>
      ${targetConfig}
...
function generateLWCFiles(
  ...
  if (type === "card" && !isStdRuntime) {
    ...
    htmlSource = generateStandardRuntimeWrapperHtml(
      item.Name,
      layoutDefinition,
      nsPrefix
    );
    jsSource = generateStandardRuntimeWrapperJs(
      lwcComponentName,
      layoutDefinition,
      nsPrefix
    );
...
function getMetaData(lwcName, updatedCardObj, type) {
  let cardObjectFields = getCardObjectFields(updatedCardObj);
  let isActive = updatedCardObj[cardObjectFields.IsActive];
  const isChildCard =
    updatedCardObj[cardObjectFields.IsChildCard] ||
    (updatedCardObj[cardObjectFields.Type] &&
      updatedCardObj[cardObjectFields.Type].toLowerCase() === "child");
  let xmlObject = cloneDeep(
    JSON.parse(updatedCardObj[cardObjectFields.PropertySetConfig])?.xmlObject
  );
  let metaObject = {};
  if (isInsidePackge) {
    metaObject.runtimeNamespace = nsPrefix;
  }
  if (isActive && xmlObject) {
    metaObject = {
      apiVersion: xmlObject.apiVersion || defaultXmlConfig.api,
      isExposed: true,
      masterLabel:
        xmlObject.masterLabel || (isActive ? updatedCardObj.Name : lwcName),
      description: xmlObject.description || ""
    };
    if (xmlObject.targets) {
      metaObject.targets = xmlObject.targets;
    } else if (!isChildCard) {
      metaObject.targets = {
        target: defaultXmlConfig.targets
      };
    }
    if (metaObject.targets) {
      metaObject.targetConfigs = xmlObject.targetConfigs
        ? xmlObject.targetConfigs
        : window.btoa(getDefaultTargetConfig(metaObject.targets?.target));
    }
  } else {
    metaObject = {
      apiVersion: defaultXmlConfig.api,
      isExposed: true,
      masterLabel: isActive ? updatedCardObj.Name : lwcName
    };
    if (isActive && !isChildCard) {
      ...
      if (!isChildCard)
        metaObject.targetConfigs = window.btoa(
          getDefaultTargetConfig(metaObject.targets?.target)
        );
    }
  }
  let lwcCmp = {
    name: lwcName,
    files: generateLWCFiles(lwcName, updatedCardObj, type, null, metaObject)
  };
  let resources = [];
    ...
    events.forEach((event, index) => {
      if (event.eventtype === "recordChange" && event.sobject) {
        ...
        @wire(getRecord , {recordId: ${
          event.recordId.indexOf("{") !== -1
            ? '"$recordId"'
            : `"${event.recordId}"`
        } , fields:"${event.sobject}.Id",`;
        wireAdapterStr =
          wireAdapterStr +
          (event.optionalFields
            ? `optionalFields: $cmp.getWireOptionalFields(data.events[${index}])`
            : "");
        wireAdapterStr =
          wireAdapterStr +
          `})
...
function getDefaultTargetConfig(targetArray = defaultXmlConfig.targets) {
  let targetConfigs = "";
  targetArray.forEach(target => {
    if (target === "lightning__AppPage")
      targetConfigs += defaultXmlConfig.appPageConfig;
    if (target === "lightning__RecordPage")
      targetConfigs += defaultXmlConfig.recordPageConfig;
  });
  return targetConfigs;
}
export {
  generateStandardRuntimeWrapperJs,
  generateStateHtml,
  generateCardHtml,
  generateCardJs,
