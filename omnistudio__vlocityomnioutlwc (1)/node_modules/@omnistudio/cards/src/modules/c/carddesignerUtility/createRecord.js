...
import {
  getCardObjectName,
  createFlexCard,
  getFileBasedFlexCards
} from "c/flexCardUtils";
import { getCardObject } from "./getCardObjectFields";
import FCCreateNewFlexCardValidationMessage from "@salesforce/label/c.FCCreateNewFlexCardValidationMessage";

let createNewRecord = async function(record) {
  let {
    name,
    title,
    author,
  ...
  //Restrict cards with same name as any filebased cards.
  const allFileBasedCards = [...(await getFileBasedFlexCards())];
  if (allFileBasedCards?.some(card => card.Name === name)) {
    // TODO - Customer facing label. Connect to Doc team to get this updated.
    throw new Error(FCCreateNewFlexCardValidationMessage);
  }
  const objName = await getCardObjectName();
  const cardObjFields = getCardObject(objName);
  const recordInput = {
    Name: name,
    ...
    [cardObjFields.DataSourceConfig]: JSON.stringify(dsConfig),
