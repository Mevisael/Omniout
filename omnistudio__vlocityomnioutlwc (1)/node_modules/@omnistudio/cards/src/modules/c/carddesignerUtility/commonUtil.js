...
import { get, set, orderBy, findIndex, cloneDeep } from "c/lodash";
import { getCardObjectFields } from "./getCardObjectFields";
import { getCardDataById } from "./getCard";
import saveCard from "./saveCard";
import { getLwcResourceByName } from "./getLwcResource.js";
import standardOmniStudioRuntime from "@salesforce/apex/FlexCardController.standardOmniStudioRuntime";
import { dsObj } from "c/flexCardUtils";
import { localStoreMethods } from "c/flexCardUtility";
import { getMetaData } from "./componentGenerator";
import { addLwcBundle } from "./deployment";
import { safePostMessage } from "c/salesforceUtils";
import getLWCByBundleId from "@salesforce/apex/FlexCardController.getLWCByBundleId";
import { updateLocalResource } from "./deployment";

let localSampleDataSourceResponse = {};

let frameInstance = null;

function isStandardRuntime() {
  const initialPromise = standardOmniStudioRuntime().then(response => {
    localStoreMethods.setItem("isStandardRuntime", JSON.stringify(response));
    return response;
  });
  let cacheddata = localStoreMethods.getItem("isStandardRuntime");
  if (cacheddata) {
...
function hasBackwardCompatibleWrapper(nameOfLwc) {
  return getLwcResourceByName(nameOfLwc)
    .then(resource => {
...
function getDatasourceList() {
  return new Promise(resolve => {
    isStandardRuntime().then(response => {
      let dsData = cloneDeep(dsObj);
      if (response) {
        let sdkIndex = findIndex(dsData.dsList, { value: "SDK" });
        if (sdkIndex > -1) {
          dsData.dsList.splice(sdkIndex, 1);
        }
        resolve(dsData);
      }
      resolve(dsData);
...
function lwcPartialUpdate(nameOfLwc, updateResources, cardObject, identifier) {
  return new Promise((resolve, reject) => {
    if (nameOfLwc && updateResources?.length && cardObject) {
      let metaObj = getMetaData(nameOfLwc, cardObject, "card");
      let resources = [...metaObj?.lwcResources?.lwcResource];
      updateResources.forEach(element => {
        const hasRes = resources.findIndex(
          ele => ele.name === (element.name || element.Name)
        );
        if (hasRes !== -1) {
          resources.splice(hasRes, 1, {
      ...
      isIframeLoaded().then(res => {
        if (res) {
          const messageObj = {
            name: "deployLightningWebComponent",
            resources: resources,
            identifier: identifier
          };
          safePostMessage(frame?.contentWindow, messageObj, "*");
...
async function handlePartialUpdateResponse(
  res,
  nameOfLwc,
  cardObj,
  updateLocalRes = true
  ...
  if (res?.result?.error) {
    rejectPromise(res.result.error);
    unsetPromise();
  } else if (res?.result?.results && nameOfLwc && cardObj) {
    const response = res.result.results[0];
    getLwcResourceByName(nameOfLwc);
    let bundle = {
      DeveloperName: nameOfLwc,
      Id: response.id,
      MasterLabel: cardObj.Name,
      NamespacePrefix: response.namespace || "c",
      ManageableState: response.manageableState || "unmanaged"
    };
    addLwcBundle(bundle);
    if (updateLocalRes) {
      const resources = await getLWCByBundleId({ Id: response.id });
      if (resources?.length) updateLocalResource(nameOfLwc, resources);
    }
    resolvePromise(true);
    unsetPromise();
  }
...
function resolvePromise(val) {
  if (promiseResolve) promiseResolve(val);
}
function rejectPromise(val) {
  if (promiseReject) promiseReject(val);
}
const iframeLoadingPromises = new Array();

let isVFPageLoaded = false;

function getVFPageLoadingStatus() {
  return isVFPageLoaded;
}

function setVFPageLoadingStatus(loadingStatus) {
  isVFPageLoaded = loadingStatus;
}

function isIframeLoaded() {
  if (iframeLoadingPromises[0]) {
    return iframeLoadingPromises[0];
  }

  const loadingPromise = new Promise(async resolve => {
    if (getVFPageLoadingStatus()) {
      resolve(true);
    } else {
      // eslint-disable-next-line @lwc/lwc/no-async-operation
      const checkInterval = setInterval(() => {
        const updatedStatus = getVFPageLoadingStatus();
        if (updatedStatus) {
  ...
  iframeLoadingPromises.push(loadingPromise);

  loadingPromise.finally(() => {
    iframeLoadingPromises.pop();
  });
...
export {
  ...
  isStandardRuntime,
  getDatasourceList,
  hasBackwardCompatibleWrapper,
  lwcPartialUpdate,
  setFrame,
  getFrame,
  handlePartialUpdateResponse,
  getVFPageLoadingStatus,
  setVFPageLoadingStatus,
  isIframeLoaded
};
