...
describe('c-omniscript-save-for-later-acknowledge', () => {
    ...
    it('should announce the status message when omniscript is saved for later for lightning theme', () => {
        saveForLater.layout = 'lightning';
        document.body.appendChild(saveForLater);

        const omniSavedHeader = saveForLater.shadowRoot.querySelector('h1.slds-card__header-title');
        expect(omniSavedHeader.getAttribute('role')).toBe('status');
    });

    it('should announce the status message when omniscript is saved for later for newport theme', () => {
        document.body.appendChild(saveForLater);

        const omniSavedHeader = saveForLater.shadowRoot.querySelector('h1.nds-card__header-title');
        expect(omniSavedHeader.getAttribute('role')).toBe('status');
    });
});
