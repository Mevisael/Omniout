...
import { timeFormatCompatability, parseChronoAttributes } from 'c/omniscriptUtils';
import tmpl from './omniscriptTime.html';
...
export default class OmniscriptTime extends OmniscriptAtomicElement {
    ...
    validateData(data) {
        ...
        if (!valid) {
            if (this.jsonDef && this._propSetMap && this._propSetMap.timeType && this._propSetMap.timeType.toUpperCase() === 'STRING') {
                valid = typeof data === 'string';
            }
            if (!valid) {
                valid = data.constructor === Date || ((asDate = new Date(data)), (useAsDate = asDate.toString() !== 'Invalid Date'), useAsDate);
            }
            if (!valid) {
        ...
        if (this.jsonDef && this._propSetMap) {
            if (!this._propSetMap.strictDateFormatting) {
                ...
                if (this._propSetMap.modelTimeFormat) {
                    this._modelTimeFormat = timeFormatCompatability(this._propSetMap.modelTimeFormat);
                }
            }
            ...
            this._messageWhenRangeOverflow = String(this.allCustomLabelsUtil.OmniTimeMinMax) + ' %startTime% - %endTime%';
