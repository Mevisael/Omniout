...
describe('c-test-block', () => {
    ...
    it('can collapse', async () => {
        ...
        const fieldset = block.shadowRoot.querySelector('fieldset');
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-await
    it('focus on input after expand', async () => {
        const mock = JSON.parse(JSON.stringify(mockJsonDef));
        mock.propSetMap.collapse = true;
        container.jsonDef = mock;
        document.body.appendChild(container);
    ...
    it('has heading level', async () => {
        ...
        const block = container.shadowRoot.querySelector('c-omniscript-block');
        const heading = block.shadowRoot.querySelector('div.slds-accordion__summary-heading');
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-await
    it('child blocks should collapse', async () => {
        const mock = JSON.parse(JSON.stringify(mockJsonDef));
        mock.newClone = true;
        container.jsonDef = mock;
        container.testFocus = true;
        ...
        const childBlocks = Array.from(block.querySelectorAll('*')).filter(element => {
            const nameAttribute = element.tagName;
            return nameAttribute && nameAttribute.toLowerCase().includes('omniscript') && nameAttribute.toLowerCase().includes('block');
        });
        expect(childBlocks).toBeTruthy();
        ...
        childBlocks.forEach(child => {
            if (child.collapse) {
                child.collapse();
                expect(child.className.includes('slds-is-open')).toBe(false);
            }
        });
        expect(section.className.includes('slds-is-open')).toBe(false);
        expect(button.getAttribute('aria-expanded')).toBe('false');
