        ...
        if (kbOptions) {
            ...
            if (kbOptions.knowledgeRecordTypeFilter) {
              if (option.typeFilter.length === 0) {
                let tempTypeFilter = "";
                for (const [recordType] of Object.entries(this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap)) {
              ...
              if (option.typeFilter.split(',').length > 1) {
                ...
                for (let i = 0; i < option.typeFilter.split(',').length; i++) {
                  if (this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[typeFilters[i]] !== undefined) {
                    const currentFieldsSplit = this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[typeFilters[i]].split(',');
                    ...
                    for (let j = 0; j < currentFieldsSplit.length; j++) {
                      tempString += typeFilters[i] + ":" + currentFieldsSplit[j] + ",";
                    }
                    option.recordTypeFilters += tempString;
                    } else {
                      const allString = ":ALL,";
                      if (this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[typeFilters[i]].length === 0) {
                        // Handle case where no fields are specified for record type, which case we have to search in all fields
                        option.recordTypeFilters = option.recordTypeFilters + typeFilters[i] + allString;
                      } else {
                        option.recordTypeFilters = option.recordTypeFilters + typeFilters[i] + ":" + this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[typeFilters[i]] + ",";
                      }
                    }
                  }  else {
                ...
                if (this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[option.typeFilter] === undefined) {
                  // Handle case where recordType is not present at setup level, so giving preference to step level and searching in all fields
                  const allString = ":ALL";
                  option.recordTypeFilters = option.typeFilter + allString;
                } else {
                  const currentFieldsSplit = this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[option.typeFilter].split(',');
                  if (currentFieldsSplit.length > 1) {
                    let tempString = '';
                    for (let i = 0; i < currentFieldsSplit.length; i++) {
                      tempString += option.typeFilter + ":" + currentFieldsSplit[i] + ",";
                    }
                    option.recordTypeFilters = tempString;
                  } else {
                    const allString = ":ALL";
                    if (this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[option.typeFilter].length === 0) {
                      // Handle case where no fields are specified for record type, which case we have to search in all fields
                      option.recordTypeFilters = option.typeFilter + allString;
                    } else {
                      option.recordTypeFilters = option.typeFilter + ":" + this._knowledgeBaseOptions.knowledgeArticleTypeQueryFieldsMap[option.typeFilter];
                    }
                  }
                }
              }
