        ...
        if (uiElements.hasOwnProperty(item)) {
            ...
            if (acUiElements.hasOwnProperty(item)) {
                ...
                if (childCompList && childCompList.length > 0) {
                    if (comp.scriptHeaderDef.allowOverwrite === true && VALIDREPEATTYPE.hasOwnProperty(childCompList[0].jsonDef.type)) {
                        ...
                        if (!valueIsNull) {
                            ...
                            if (incomingElementCount < childCompList.length) {
                                ...
                                for (let i = childCompList.length - 1; i >= incomingElementCount; i--) {
                            ...
                            //add
                                ...
                                if (compListEmpty) {
                                    childCompList[0].handleAdd(API, !bDataArray ? data[item] : data[item][0]);
                                }

                                // childCompList.length is always > 0
                                const componentIndexToPerformAdd = childCompList.length - 1;
                                for (let i = incomingElementCount - 1; i >= childCompList.length; i--) {
                                    childCompList[componentIndexToPerformAdd].handleAdd(API, data[item][i]);
                                }
                            }
                        }
                        ...
                        if (!bDataArray) {
                            ...
                            let maxUpdateCount = Math.min(incomingElementCount, childCompList.length);
                    ...
                    else if (childCompList[0].jsonDef.propSetMap.repeat || VALIDREPEATTYPE.hasOwnProperty(childCompList[0].jsonDef.type)) {
