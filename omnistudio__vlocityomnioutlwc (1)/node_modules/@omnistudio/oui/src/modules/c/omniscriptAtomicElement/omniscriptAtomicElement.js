...
export default class OmniscriptAtomicElement extends HasValidation(OmniscriptBaseElement) {
    ...
    prepareCustomMask(mask) {
        ...
        this._maskProperties.thousandsSeparator = group ? thousandSeparator : this._maskProperties.thousandsSeparator;
        this._maskProperties.radix = radix ? mask[radix] : this._maskProperties.radix;

        const radixToEnd = radix ? mask.length - radix - 1 : 0;

        this._maskProperties.scale = radixToEnd >= 0 ? radixToEnd : this._maskProperties.scale;
    ...
    prepareIMaskProperties(mask, thousand, decimal) {
        ...
        if (mask || typeof mask === 'number') {
            ...
            this._maskProperties.padFractionalZeros = typeof mask === 'number' && mask > 0;
    ...
    getImaskCurrencyAttributes() {
        ...
        if (!this.childInput) {
            ...
            const propMask = this._propSetMap.mask == null ? DEFAULT_CURRENCY_SCALE : this._propSetMap.mask;

            // thousand separator (group), decimal separator (radix)
            this.prepareIMaskProperties(propMask, currencyInfo.group, currencyInfo.decimal);
    ...
    getImaskNumberAttributes() {
        ...
        if (!this.childInput) {
            ...
            this.prepareIMaskProperties(this._propSetMap.mask, thousandsSeparator, radix);
            imaskAttributes = {
                // TODO: move convert imask options to single attributes.
                // eslint-disable-next-line no-new-wrappers
                mask: new Number(),
    ...
    getCurrencyCode() {
        return this.jsonData.OmniScriptCurrencyCode || this.scriptHeaderDef.propSetMap.currencyCode || this.jsonData.userCurrencyCode;
    }
    ...
    getCurrencySymbol() {
        ...
        return this._propSetMap.displayCurrencyCode ? currencyInfo.code : currencyInfo.text;
    }
