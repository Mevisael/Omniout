...
import OmniscriptDrTransformAction from 'c/omniscriptDrTransformAction';
import * as Link from 'c/omniscriptRestApi';
import { delay } from 'c/asyncUtils';

describe('c-omniscript-dr-transform-action', () => {
    ...
    beforeEach(() => {
        element = createElement('c-omniscript-dr-transform-action', {
            is: OmniscriptDrTransformAction,
        });
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-await
    it('should have an api response after executing valid and successful transform action', async () => {
        expect.assertions(1);
        document.body.appendChild(element);
        const input = element.shadowRoot.querySelector('c-button');
        await delay(0);
        ...
        return new Promise(resolve => {
            element.addEventListener('omniactionbtn', event => {
                expect(event.detail.apiResponse).toEqual(personInfo);
                resolve();
            });
