...
import { sldsContainerClassTmpl, ndsContainerClassTmpl, DEFAULT_CANCEL, LAUNCHER_ACTION, EMBEDDED_SOURCES } from './omniscriptHeaderConstants';
...
export default class OmniscriptHeader extends OmniscriptGroupElement {
    ...
    connectedCallback() {
        ...
        if (Object.keys(this._urlParams).length === 0) {
            this._urlParams = normalizeParams(
                this.currentPageReference?.state ? this.currentPageReference.state : parseParams(window.location.search),
            );
        }
        ...
        if (this._multiLang) {
            if (multiLangParams.LanguageCode != null) {
                // prefill containing languageCode
                this._languageCode = multiLangParams.LanguageCode;
            } else if (this._urlParams.hasOwnProperty('LanguageCode') && this._urlParams.LanguageCode != null) {
                this._languageCode = this._urlParams.LanguageCode;
            } else if (this.langCode != null) {
                // omniscript preview's language selection
                this._languageCode = this.langCode;
            }
        }
        // user's language code from i18n
        // will work in multi-language and single-language mode
        if (UserLanguageCode != null && this._languageCode == null) {
            this._languageCode = UserLanguageCode;
        }
        this.setLanguageDirection(this._languageCode);
        ...
        Promise.all(promiseList)
            .then(() => {
                if (this._bValidOs && ((this.jsonDef.children && this.jsonDef.children.length > 0) || this.resume)) {
                    ...
                    // Extract the metrics
                    this._metricsGenerator = new OmniScriptMetricsGenerator({
                        jsonDef: this.jsonDef,
                        theme: this._theme,
                        isActive: this._isActiveOs,
                        isPreview: this._bPreview,
                        isDesignMode: this.runMode === RUN_MODES.DESIGNER,
                    });

                    // Start metrics should not be run in designer mode
                    this.generateMetrics(this._metricsGenerator, 'omniScriptStart');
    ...
    generateMetrics(metricsGenerator, operation, index) {
        // added a flag to allow users to turn off logging metrics
        if (metricsGenerator && this.jsonDef.propSetMap.logMetrics !== false) {
            metricsGenerator.save(this._metricsGenerator.get(operation, index));
        }
    }
    ...
    initScriptHeaderDef() {
        ...
        this.scriptHeaderDef = {
            ...
            allowOverwrite: this.jsonDef.propSetMap.allowOverwrite === undefined || this.jsonDef.propSetMap.allowOverwrite,
        };
        ...
        if (this._systemInfo) {
            ...
            //Passing Omniscript Id in the data json
            this._systemInfo.omniProcessId = this.jsonDef.sOmniScriptId;
            //Passing localTimeZone in the data json
            this._systemInfo.localTimeZoneName = dayjs.tz.guess();
        }
    ...
    initJsonDefResponse() {
        ...
        // update the data json debugger when debug mode (running in designer)
        if (this.runMode === RUN_MODES.DEBUG) {
            ...
            ['previous', 'next', 'save'].forEach(btnType => {
                ...
                if (newWidth) {
                    if (newWidth === '0') {
                        ...
                        this.navButton[btnType].classes = existingClasses.concat([`${this._theme}-size_0-of-1`]).join(' ');
                        this.navButton = Object.assign({}, this.navButton); // refresh
                    } else {
                        ...
                        if (this._isDesignMode) {
                            ...
                            this.navButton[btnType].classes = existingClasses
                                .filter(clz => {
                                    return !/-medium-size_/.test(clz) || !/-size_0-of-1/;
                                })
                            ...
                            if (oldWidth !== newWidth || this._initialHeaderRender || currentIndex === 0) {
                                ...
                                this.navButton[btnType].classes = this.navButton[btnType].classes.split(`${this._theme}-size_0-of-1`).join('');
    ...
    getAdjacentIndex(currentIndex, direction = 'prev') {
        ...
        while (
            (this.jsonDef.children[currentIndex] && this.jsonDef.children[currentIndex].type !== 'Step') ||
            (this.jsonDef.children[currentIndex] &&
                this.jsonDef.children[currentIndex].bShow !== undefined &&
                this.jsonDef.children[currentIndex].bShow !== true)
        ) {
            currentIndex = currentIndex + incrementor;
        }
    ...
    navigateTo(nextIndex, currentIndex = 0) {
        ...
        if (canNavigate && this.jsonDef.children[nextIndex] && this.jsonDef.children[nextIndex].type === 'Cancel Action') {
            // OmniScript metrics
            this.generateMetrics(this._metricsGenerator, 'navigateIntoAction', nextIndex);
        ...
        //* Navigation logic - when next element is not last and no error messages are present
            ...
            if (bShow === undefined || bShow === true) {
                ...
                if (this.jsonDef.children[nextIndex].type === 'Step') {
                    ...
                    // OmniScript metrics
                    const metric = this.runMode === RUN_MODES.DESIGNER ? 'designStep' : 'navigateIntoStep';
                    this.generateMetrics(this._metricsGenerator, metric, nextIndex);
            ...
            //* Navigation logic - navigate to the next index if step/action is hidden
                ...
                // OmniScript metrics for hidden actions / steps, off by default in 248
                if (this.jsonDef.propSetMap.logMetricsHidden === true) {
                    this.generateMetrics(this._metricsGenerator, 'navigateIntoHiddenStepOrAction', nextIndex);
                }
    ...
    resetOmniScriptPosition(nextIndex, currentIndex) {
        if (
            ...
            if (nextIndex < currentIndex || nextIndex > this.scriptHeaderDef.firstStepIndex) {
                ...
                const scrollBehavior = this.jsonDef.propSetMap.scrollBehavior === 'smooth' ? 'smooth' : 'auto';

                // When Omniscript is embedded in a flyout modal or utility bar or communities, scrolling will
                // need to be performed by querying element as the window is outside the scope. All other
                // conditions will default to using window scroll
                if (
                    (this.scriptHeaderDef.isCommunity &&
                        this.jsonDef.propSetMap.scrollToTopWhenNavigatingBetweenStepsOnExperienceCloudSites !== true) ||
                    (this.jsonDef.response && EMBEDDED_SOURCES.includes(this.jsonDef.response.omniscriptEmbeddedSource))
                ) {
                    const articleComp = this.template.querySelector('.omniscript-article');

                    if (articleComp) {
                        articleComp.scrollIntoView({
                            behavior: scrollBehavior,
                            inline: 'start',
                        });
                    }
                } else {
                    // Within communities, users are able to configure to scroll to the top of the page window
                    window.scrollTo({
                        top:
                            this.scriptHeaderDef.isCommunity &&
                            this.jsonDef.propSetMap.scrollToTopWhenNavigatingBetweenStepsOnExperienceCloudSites === true
                                ? 0
                                : this._resetPositionComp.offsetTop,
                        behavior: scrollBehavior,
                    });
                }
            }
    ...
    handleActionExecution(comp, index) {
        ...
        this.spinnerMessage = this?.allCustomLabelsUtil?.[setSpinnerActionMessage(element)] || setSpinnerActionMessage(element);
        ...
        return Promise.resolve(this.cancelAction)
            ...
            .catch(reason => {
                if (reason !== OmniscriptCancelAction.CANCEL_ABORTED && reason !== OmniscriptCancelAction.CANCEL_DISABLED) {
                    ...
                    if (this.template.querySelector('.omniscript-sfl-actions .omniscript-btn-save-for-later:first-child')) {
                        this.template.querySelector('.omniscript-sfl-actions .omniscript-btn-save-for-later:first-child').focus();
                    }
                }
            });
    }
    ...
    performSaveForLater(auto) {
        ...
        saveForLaterUtil(
            ...
            .catch(error => {
                this.compLoaded = true;
                this.showComp(true);
                const errorMessage = handleErrorReplace(error.message, null, this.scriptHeaderDef);
                this.notifySaveForLaterError({
                    type: error.type,
    ...
    notifySaveForLaterError(error) {
        ...
        if (
            error.type.indexOf('PlatformSecurityChecker.PlatformAccessException') === -1 &&
            error.type.indexOf('OmniScriptSecurityException') === -1
        ) {
            message += '<br><br>' + this.allCustomLabelsUtil.OmniSavedFailed;
        }

        if (error.status === 500) {
            message = this.allCustomLabelsUtil.OmniSavedFailed;
        }
    ...
    showComp(isVisible = true) {
        ...
        this.containerClasses += this.jsonDef.propSetMap.allowLongLabels ? ' allow-long-labels ' : '';
    }
    ...
    checkResume() {
        ...
        if (!this.resume) {
            const instanceId = normalizeParams(parseParams(window.location.search))?.instanceId;
            if (instanceId) {
                this.resume = true;
     ...
     */
    setLanguageDirection(languageCode) {
        if (languageCode) {
            this._isRTL = isRTL(languageCode);
        }
        this._dir = this._isRTL ? 'rtl' : 'ltr';
    }
}
