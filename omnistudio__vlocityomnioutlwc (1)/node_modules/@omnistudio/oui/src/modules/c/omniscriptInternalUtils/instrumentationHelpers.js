 ...
 * Build: v234.0.0
 */
import { logOmniScriptMetrics } from 'c/omniscriptRestApi';
...
export class OmniScriptMetricsGenerator {
    ...
    constructor({ jsonDef, theme, isActive, isPreview, isDesignMode }) {
        ...
        this._metricsHeader = this._buildMetricsHeader(jsonDef, theme, isActive, isPreview);
    ...
    _generate(jsonDef) {
        ...
        this._extractChildrenMetrics(true, jsonDef.children, uniqueElements, childFlexCards, elementsMap, conditionalElementsMap);

        // Save the OS metrics
        this._omniscriptMetrics = this._buildMetrics(uniqueElements, childFlexCards, elementsMap, conditionalElementsMap);
    }
     ...
     */
    _extractChildrenMetrics(isRoot, children, parentUniqueElements, parentChildFlexCards, parentElementsMap, parentConditionalElementsMap) {
        ...
        children.forEach((child, idx) => {
            ...
            this._extractChildMetrics(child, uniqueElements, childFlexCards, elementsMap, conditionalElementsMap);

            if (isRoot) {
                // Save the metrics in the cache
                this._cachedMetrics.set(idx, {
                    stepVisitCount: 0,
                    metrics: this._buildMetrics(uniqueElements, childFlexCards, elementsMap, conditionalElementsMap),
                });
            }
     ...
     * @returns
     */
    _buildMetrics(uniqueElements, childFlexCards, elementsMap, conditionalElementsMap) {
        return {
     ...
     */
    _extractChildMetrics(child, uniqueElements, childFlexCards, elementsMap, conditionalElementsMap) {
        if (child.type) {
        ...
        if (child.propSetMap) {
            if (child.propSetMap?.show && child.propSetMap.show !== null && child.type) {
                conditionalElementsMap.set(child.type, this._getMetricsValueFromMap(conditionalElementsMap, child.type) + 1);
            }
        }
        ...
        if (child.eleArray && Array.isArray(child.eleArray)) {
            child.eleArray.forEach(item => {
                ...
                if (item.propSetMap) {
                    if (item.propSetMap?.show && item.propSetMap.show !== null && item.type) {
                        conditionalElementsMap.set(item.type, this._getMetricsValueFromMap(conditionalElementsMap, item.type) + 1);
                    }

                    if (item.type === 'Custom Lightning Web Component' && (item.propSetMap.lwcName || '').startsWith('cf')) {
                        childFlexCards.add(item.propSetMap.lwcName);
                    }
                }

                this._extractChildrenMetrics(false, item.children, uniqueElements, childFlexCards, elementsMap, conditionalElementsMap);
            });
        }

        this._extractChildrenMetrics(false, child.children, uniqueElements, childFlexCards, elementsMap, conditionalElementsMap);
    }
